...
<div>
    ...
    <div>
        ...
        <div>
            ...
            <input type="number" value="<?= $row['jum']?>" class="form-control" required readonly>
        </div>
        <div>
            <label for="">Jumlah Baik</label>
            <input type="number" name="jumlah" value="<?= $row['jumlah_baik']?>" class="form-control" required autofocus>
        </div>
        <div>
                <label for="">Keterangan</label>
                <textarea name="keterangan" class="form-control" id="" cols="30" rows="10" readonly><?= $row['keterangan']?></textarea>
        </div>
        <br>
        <div>
...
<?php
    ...
    if(isset($_POST['simpan'])){
        ...
        mysqli_query($koneksi,"UPDATE kondisi_barang SET jumlah_baik ='$jumlah' WHERE id_kondisi = '$id'");
        ?>
        <script>
            swal({
  title: "Success!",
