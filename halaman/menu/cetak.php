...
$query = mysqli_query($koneksi, "SELECT barang.nm_barang,kategori.nm_kategori,pesanan.jumlah,pesanan.jumlah*pesanan.harga as harga, jual_beli.jual as ha,merek FROM `pesanan` JOIN `order` ON `order`.`id_order` = pesanan.id_order JOIN barang ON barang.id_barang = pesanan.id_barang JOIN jual_beli ON barang.id_barang = jual_beli.id_barang JOIN kategori ON kategori.id_kategori = barang.id_kategori WHERE pesanan.id_order = '$id_order'");
